...
class TestQueryOrchestrationTypes:
    """Test query orchestration-related data types."""

    def test_processed_query_defaults_are_backward_compatible(self):
        """Test ProcessedQuery defaults keep the current direct-query behavior."""
        ...
        assert processed.expanded_terms == []
        assert processed.orchestration_mode == "direct"
        assert processed.sub_queries == []
        assert processed.planner_reason is None
        assert processed.planner_confidence is None

    def test_processed_query_serialization_with_orchestration_fields(self):
        """Test ProcessedQuery serializes extended orchestration fields."""
        original = ProcessedQuery(
            original_query="比较 Azure 和 OpenAI 的配置差异",
            keywords=["比较", "Azure", "OpenAI", "配置", "差异"],
            filters={"collection": "docs"},
            expanded_terms=["微软云", "Azure OpenAI"],
            orchestration_mode="decomposition",
            sub_queries=["Azure 配置步骤", "OpenAI 配置步骤"],
            planner_reason="原始问题包含两个并列对比对象",
            planner_confidence=0.92,
        )

        data = original.to_dict()
        restored = ProcessedQuery.from_dict(data)

        assert restored.original_query == original.original_query
        assert restored.expanded_terms == ["微软云", "Azure OpenAI"]
        assert restored.orchestration_mode == "decomposition"
        assert restored.sub_queries == ["Azure 配置步骤", "OpenAI 配置步骤"]
        assert restored.planner_reason == "原始问题包含两个并列对比对象"
        assert restored.planner_confidence == 0.92

    def test_sub_query_plan_serialization(self):
        """Test SubQueryPlan can be serialized and restored."""
        sub_query = SubQueryPlan(
            query="Azure 配置步骤",
            intent="提取 Azure 路径",
            order=1,
        )
        ...
        restored = SubQueryPlan.from_dict(data)

        assert restored.query == "Azure 配置步骤"
        assert restored.intent == "提取 Azure 路径"
        assert restored.order == 1

    def test_query_plan_serialization_with_sub_queries(self):
        """Test QueryPlan handles nested sub-query plan serialization."""
        plan = QueryPlan(
            mode="decomposition",
            reason="问题包含多个需要独立检索的子意图",
            expanded_terms=["Azure OpenAI"],
            sub_queries=[
                SubQueryPlan(query="Azure 配置步骤", intent="提取 Azure 路径", order=1),
                SubQueryPlan(query="OpenAI 配置步骤", intent="提取 OpenAI 路径", order=2),
            ],
            confidence=0.88,
            used_fallback=False,
        )
        ...
        assert restored.mode == "decomposition"
        assert restored.reason == "问题包含多个需要独立检索的子意图"
        assert restored.expanded_terms == ["Azure OpenAI"]
        assert len(restored.sub_queries) == 2
        assert restored.sub_queries[0].query == "Azure 配置步骤"
        assert restored.sub_queries[1].intent == "提取 OpenAI 路径"
