...
from src.core.types import ProcessedQuery, QueryPlan, SubQueryPlan
...
class TestQueryPlanIntegration:
    """Test QueryProcessor integration with query orchestration plans."""

    def test_process_applies_synonym_expanded_terms(self):
        """Test synonym plans populate expanded_terms and metadata."""
        processor = QueryProcessor()
        plan = QueryPlan(
            mode="synonym",
        ...
        result = processor.process("RAG 是什么？", query_plan=plan)

        assert result.orchestration_mode == "synonym"
        assert result.expanded_terms == ["检索增强生成", "Retrieval Augmented Generation"]
        assert result.planner_reason == "query contains acronym"
        assert result.planner_confidence == 0.86

    def test_process_applies_decomposition_sub_queries(self):
        """Test decomposition plans populate sub_queries without expanded terms."""
        processor = QueryProcessor()
        plan = QueryPlan(
            mode="decomposition",
            reason="query has multiple intents",
            sub_queries=[
                SubQueryPlan(query="Azure 配置步骤", intent="Azure path", order=1),
                SubQueryPlan(query="OpenAI 配置步骤", intent="OpenAI path", order=2),
            ],
        )

        result = processor.process("比较 Azure 和 OpenAI 的配置差异", query_plan=plan)

        assert result.orchestration_mode == "decomposition"
        assert result.sub_queries == ["Azure 配置步骤", "OpenAI 配置步骤"]
