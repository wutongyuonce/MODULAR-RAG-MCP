...
class TestResolveImagePath:
    ...
    def test_resolve_with_explicit_path(self, assembler, temp_image_dir):
        ...
        assembler = MultimodalAssembler(allowed_images_root=temp_image_dir.parent)
    ...
    def test_resolve_fallback_convention_path(self, tmp_path, sample_png_bytes):
        """Test fallback to convention-based path."""
        images_root = tmp_path / "data" / "images"
        collection = "test_collection"
        expected_path = images_root / collection / "conv_img.png"
        expected_path.parent.mkdir(parents=True, exist_ok=True)
        expected_path.write_bytes(sample_png_bytes)

        assembler = MultimodalAssembler(allowed_images_root=images_root)
        ref = ImageReference(image_id="conv_img")

        with patch(
            "src.core.response.multimodal_assembler.resolve_path",
            side_effect=lambda relative_path: tmp_path / relative_path,
        ):
    ...
    def test_resolve_nonexistent_path(self, tmp_path):
        ...
        assembler = MultimodalAssembler(allowed_images_root=tmp_path)
        ref = ImageReference(
    ...
    def test_resolve_rejects_path_outside_allowed_root(self, temp_image_dir):
        """Reject metadata paths that escape the configured image sandbox."""
        outside_file = temp_image_dir.parent / "secrets.txt"
        outside_file.write_text("secret")

        assembler = MultimodalAssembler(allowed_images_root=temp_image_dir)
        ref = ImageReference(image_id="secret", file_path=str(outside_file))

        assert assembler.resolve_image_path(ref) is None
...
class TestLoadImage:
    ...
    @pytest.mark.parametrize(
        ...
        [("img001.png", "image/png"), ("img002.jpg", "image/jpeg")],
    )
    def test_load_image_supported_formats(
        self,
        temp_image_dir,
        file_name: str,
        expected_mime: str,
    ):
        """Test loading supported image formats from the allowed image root."""
        assembler = MultimodalAssembler(allowed_images_root=temp_image_dir.parent)
        image_path = temp_image_dir / file_name

        content = assembler.load_image(str(image_path))

        assert content is not None
        assert content.mime_type == expected_mime
        assert content.image_id == image_path.stem
        assert base64.b64decode(content.data) == image_path.read_bytes()
    
    def test_load_nonexistent_file(self, tmp_path):
        """Test loading returns None for missing file."""
        assembler = MultimodalAssembler(allowed_images_root=tmp_path)
        content = assembler.load_image(str(tmp_path / "missing.png"))
    ...
    def test_load_empty_file(self, tmp_path):
        ...
        content = assembler.load_image(str(empty_file))

        assert content is None

    def test_load_image_rejects_path_outside_allowed_root(self, temp_image_dir):
        """Reject file reads outside the allowed image root."""
        outside_file = temp_image_dir.parent / "not_an_image.txt"
        outside_file.write_text("secret")

        assembler = MultimodalAssembler(allowed_images_root=temp_image_dir)

        assert assembler.load_image(str(outside_file)) is None
...
class TestAssembleForResult:
    ...
    def test_assemble_includes_caption(self, temp_image_dir, sample_png_bytes):
        ...
        assembler = MultimodalAssembler(
            ...
            allowed_images_root=temp_image_dir.parent,
        )
        result = RetrievalResult(
            chunk_id="test",
            score=0.9,
