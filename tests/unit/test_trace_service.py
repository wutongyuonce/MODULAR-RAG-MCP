            ...
            "stages": [
                {
                    ...
                    "elapsed_ms": 10.0,
                    "data": {"mode": "decomposition", "sub_query_count": 2},
                },
                {"stage": "dense_retrieval", "elapsed_ms": 25.0, "data": {}},
                {"stage": "rerank", "elapsed_ms": 25.0, "data": {}},
            ],
            "metadata": {},
        },
        {
...
class TestTraceService:
    ...
    def test_get_stage_timings_preserves_query_orchestration_stage(self, traces_file):
        svc = TraceService(traces_file)
        t = svc.get_trace("t2")
        timings = svc.get_stage_timings(t)
        orchestration = next(stage for stage in timings if stage["stage_name"] == "query_orchestration")
        assert orchestration["elapsed_ms"] == 10.0
        assert orchestration["data"]["mode"] == "decomposition"
        assert orchestration["data"]["sub_query_count"] == 2
