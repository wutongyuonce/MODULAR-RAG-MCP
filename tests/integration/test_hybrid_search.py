...
class QueryAwareDenseRetriever(MockDenseRetriever):
    """Dense retriever that returns results based on the query string."""

    def __init__(self, query_to_results: Dict[str, List[RetrievalResult]]):
    ...
    def retrieve(
        ...
        self.last_top_k = top_k
        self.last_filters = filters
        return self.query_to_results.get(query, [])[:top_k]


class KeywordAwareSparseRetriever(MockSparseRetriever):
    """Sparse retriever that routes results based on keyword content."""

    def __init__(self, keyword_to_results: Dict[str, List[RetrievalResult]]):
        super().__init__(results=[])
        self.keyword_to_results = keyword_to_results
    ...
    def retrieve(
        ...
        self.last_top_k = top_k
        self.last_collection = collection
        for key, results in self.keyword_to_results.items():
            if key in keywords:
...
class TestHybridSearchBasic:
    ...
    def test_synonym_query_plan_only_expands_sparse_path(
        self,
        query_processor: QueryProcessor,
        rrf_fusion: RRFFusion,
        sample_dense_results: List[RetrievalResult],
        ...
        """Test synonym expansion augments sparse keywords but keeps dense query unchanged."""
        dense = MockDenseRetriever(results=sample_dense_results)
        sparse = MockSparseRetriever(results=sample_sparse_results)
        ...
        assert result.processed_query is not None
        assert result.processed_query.orchestration_mode == "synonym"
        assert result.processed_query.expanded_terms == [
            "检索增强生成",
            "Retrieval Augmented Generation",
        ]
        assert dense.last_query == "RAG 是什么？"
        assert "RAG" in sparse.last_keywords
        assert "检索增强生成" in sparse.last_keywords
        assert "Retrieval Augmented Generation" in sparse.last_keywords

    def test_decomposition_query_plan_runs_sub_queries_and_merges(
        self,
        query_processor: QueryProcessor,
    ):
        """Test decomposition executes multiple sub-queries and merges deduplicated results."""
        dense = QueryAwareDenseRetriever(
            {
                "Azure 配置步骤": [
                    RetrievalResult(
                        chunk_id="azure_dense",
                        score=0.95,
                        text="Azure path",
                    ...
                    RetrievalResult(
                        ...
                        score=0.90,
                        text="Shared config chunk",
                        metadata={"source_path": "docs/shared.pdf"},
                    ),
                ],
                "OpenAI 配置步骤": [
                    RetrievalResult(
                        chunk_id="openai_dense",
                        score=0.96,
                        text="OpenAI path",
                        metadata={"source_path": "docs/openai.pdf"},
                    ),
                    RetrievalResult(
                        chunk_id="shared_chunk",
                        score=0.89,
        ...
        sparse = KeywordAwareSparseRetriever(
            {
                "Azure": [
                    RetrievalResult(
                        chunk_id="shared_chunk",
                    ...
                    RetrievalResult(
                        chunk_id="azure_sparse",
                        score=7.5,
                        text="Azure sparse hit",
                        metadata={"source_path": "docs/azure-bm25.pdf"},
                    ),
                ],
                "OpenAI": [
                    RetrievalResult(
                    ...
                    RetrievalResult(
                        chunk_id="openai_sparse",
                        score=7.4,
                        text="OpenAI sparse hit",
                        metadata={"source_path": "docs/openai-bm25.pdf"},
                    ),
                ],
            }
        )
        settings = SimpleNamespace(
            retrieval=SimpleNamespace(
                query_orchestration=SimpleNamespace(
        ...
        plan = QueryPlan.from_dict(
            {
                ...
                "sub_queries": [
                    {"query": "Azure 配置步骤", "intent": "Azure path", "order": 1},
                    {"query": "OpenAI 配置步骤", "intent": "OpenAI path", "order": 2},
                ],
            }
        )
        ...
        assert isinstance(result, HybridSearchResult)
        assert result.processed_query is not None
        assert result.processed_query.orchestration_mode == "decomposition"
        assert result.sub_query_results is not None
        ...
        assert "azure_dense" in chunk_ids
        assert "openai_dense" in chunk_ids
        assert chunk_ids.count("shared_chunk") == 1

    def test_decomposition_tolerates_failed_sub_query(
        self,
        query_processor: QueryProcessor,
    ):
        """Test one failed sub-query does not abort the full decomposition search."""

        class PartiallyFailingDenseRetriever(QueryAwareDenseRetriever):
            def retrieve(
                self,
                query: str,
                top_k: int = 10,
                ...
                if query == "OpenAI 配置步骤":
                    raise RuntimeError("OpenAI dense path failed")
                return super().retrieve(query, top_k=top_k, filters=filters, trace=trace)

        dense = PartiallyFailingDenseRetriever(
            {
                "Azure 配置步骤": [
                    RetrievalResult(
                        chunk_id="azure_dense",
        ...
        assert result.sub_query_results is not None
        assert any(item["used_fallback"] for item in result.sub_query_results)
