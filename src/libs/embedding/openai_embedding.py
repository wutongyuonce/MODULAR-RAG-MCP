...
class OpenAIEmbedding(BaseEmbedding):
    ...
    like text-embedding-ada-002. It can also work with OpenAI-compatible
    providers via ``base_url``.
    
    Attributes:
        api_key: The API key for authentication.
        model: The model identifier to use.
        dimensions: Optional dimension reduction for models/providers that
    ...
    def embed(
        ...
        **kwargs: Any,
        ...
        # Pass through dimensions for OpenAI text-embedding-3-* and
        # compatible providers such as DashScope text-embedding-v3/v4.
        # Older models such as text-embedding-ada-002 do not support it.
        dimensions = kwargs.get("dimensions", self.dimensions)
        if dimensions is not None and self._supports_dimensions_parameter():
    ...
    def _supports_dimensions_parameter(self) -> bool:
        """Return whether the configured model likely supports ``dimensions``.

        We explicitly allow the OpenAI text-embedding-3 family and common
        OpenAI-compatible model families such as DashScope text-embedding-v3/v4.
        """
        model_name = (self.model or "").lower()
        return (
            model_name.startswith("text-embedding-3")
            or model_name.startswith("text-embedding-v3")
            or model_name.startswith("text-embedding-v4")
        )
