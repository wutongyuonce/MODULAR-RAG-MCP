...
from src.core.security import resolve_path_under_base
...
class OpenAIVisionLLM(BaseVisionLLM):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        allowed_image_root = kwargs.pop("allowed_image_root", None)
        self.allowed_image_root = (
            Path(allowed_image_root).resolve()
            if allowed_image_root is not None
            else resolve_path("data/images")
        )
    ...
    def _read_local_image_bytes(self, image_path: str | Path) -> bytes:
        """Read local image bytes from the allowed image sandbox only."""
        try:
            safe_path = resolve_path_under_base(image_path, self.allowed_image_root)
        except ValueError as e:
            raise OpenAIVisionLLMError(f"[OpenAI Vision] Invalid image path: {e}") from e

        try:
            return safe_path.read_bytes()
        except Exception as e:
            raise OpenAIVisionLLMError(
                f"[OpenAI Vision] Failed to read image file {safe_path}: {e}"
            ) from e
    ...
    def preprocess_image(
        ...
        if image.data:
            ...
            image_bytes = self._read_local_image_bytes(image.path)
        elif image.base64:
            return image
        else:
            return image
