...
def render() -> None:
    ...
    for idx, trace in enumerate(traces):
        ...
        with st.expander(expander_title, expanded=(idx == 0)):
            ...
            orchestration_d = (stages_by_name.get("query_orchestration", {}).get("data") or {})
            ...
            orchestration_mode = orchestration_d.get("mode", meta.get("orchestration", {}).get("orchestration_mode", "direct"))
            ...
            with rc6:
                st.metric("Mode", orchestration_mode)

            # ── Diagnostic hints ───────────────────────────────
            _render_diagnostics(
                stages_by_name, dense_d, sparse_d,
            ...
            main_stage_names = ("query_processing", "query_orchestration", "dense_retrieval", "sparse_retrieval", "fusion", "rerank")
            ...
            if "query_orchestration" in stages_by_name:
                tab_defs.append(("🧭 Query Orchestration", "query_orchestration"))
            ...
            if tab_defs:
                ...
                for tab, (_, key) in zip(tabs, tab_defs):
                    with tab:
                        ...
                        elif key == "query_orchestration":
                            _render_query_orchestration_stage(data)
...
def _evaluate_single_trace(
    ...
    try:
        ...
        # Re-run retrieval
        collection = meta.get("collection", settings.vector_store.collection_name)
...
def _retrieve_chunks(
    ...
    try:
        ...
        safe_collection = validate_collection_name(collection)
        ...
        bm25_indexer = BM25Indexer(index_dir=str(resolve_path(f"data/db/bm25/{safe_collection}")))
...
def _render_query_processing_stage(data: Dict[str, Any]) -> None:
    ...
    if expanded_terms:
        ...
        st.markdown(" · ".join(f"`{term}`" for term in expanded_terms))

    if data.get("orchestration_mode"):
        st.markdown(f"**Orchestration Mode:** `{data.get('orchestration_mode')}`")
    if data.get("planner_reason"):
        st.caption(f"Planner Reason: {data.get('planner_reason')}")


def _render_query_orchestration_stage(data: Dict[str, Any]) -> None:
    """Render Query Orchestration stage details."""
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        st.metric("Mode", data.get("mode", "—"))
    with c2:
        st.metric("Planner", data.get("planner_provider", "—"))
    with c3:
        st.metric("Sub Queries", data.get("sub_query_count", len(data.get("sub_queries", []))))
    with c4:
        st.metric("Fallback", "Yes" if data.get("used_fallback") else "No")
    ...
    if sub_queries:
        st.markdown("**Sub Query Execution**")
        for idx, item in enumerate(sub_queries, start=1):
            query = item.get("query", "—")
            intent = item.get("intent", "")
            result_count = item.get("result_count", "—")
            used_fallback = item.get("used_fallback", False)
            header = f"#{idx} `{query}` · results={result_count}"
            if used_fallback:
                header += " · fallback"
            with st.expander(header, expanded=False):
                if intent:
                    st.caption(f"Intent: {intent}")
                if item.get("dense_error"):
                    st.warning(f"Dense Error: {item.get('dense_error')}")
                if item.get("sparse_error"):
                    st.warning(f"Sparse Error: {item.get('sparse_error')}")
                chunks = item.get("chunks", [])
                if chunks:
                    _render_chunk_list(chunks, prefix=f"sub_query_chunk_{idx}")
                else:
                    st.info("No results recorded for this sub-query.")
