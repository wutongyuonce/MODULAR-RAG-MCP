...
if TYPE_CHECKING:
    from src.core.query_engine.hybrid_search import HybridSearch, HybridSearchResult
    from src.core.query_engine.query_orchestrator import QueryOrchestrator
...
class QueryKnowledgeHubTool:
    ...
    def settings(self) -> Settings:
        """Get settings, always reloading from disk for hot-config support."""
    ...
    def _ensure_initialized(self, collection: str) -> None:
        ...
        rerank_enabled = self.settings.rerank.enabled and self.settings.rerank.provider != "none"
        if self._reranker is None or (not self._reranker.is_enabled and rerank_enabled):
        ...
        self._query_orchestrator = QueryOrchestrator(settings=self.settings)
    ...
    async def execute(
        ...
        effective_collection = validate_collection_name(
            collection or self.config.default_collection
        )
        ...
        try:
            ...
            response.metadata.update(
                self._build_orchestration_metadata(search_result)
            )
            ...
            trace.metadata["orchestration"] = self._build_orchestration_metadata(
                search_result
            )
    ...
    def _perform_search(
        ...
        Returns:
            HybridSearchResult with orchestration metadata.
        """
        ...
        try:
            ...
            if self._query_orchestrator is not None:
                processor = getattr(self._hybrid_search, "query_processor", None)
                preprocessed = processor.process(query) if processor is not None else None
                query_plan = self._query_orchestrator.plan(
                    query=query,
                    keywords=preprocessed.keywords if preprocessed is not None else None,
                    filters=preprocessed.filters if preprocessed is not None else None,
                    top_k=initial_top_k,
    ...
    def _build_orchestration_metadata(
        ...
        """Build response metadata describing query orchestration."""
        processed = search_result.processed_query
        sub_query_results = search_result.sub_query_results or []
        return {
            "orchestration_mode": (
                processed.orchestration_mode if processed is not None else "direct"
            ),
            "expanded_terms": (
                processed.expanded_terms if processed is not None else []
            ),
            "sub_query_count": len(sub_query_results),
            "sub_queries": sub_query_results,
            "used_fallback": search_result.used_fallback,
            "planner_reason": (
                processed.planner_reason if processed is not None else None
            ),
            "planner_confidence": (
                processed.planner_confidence if processed is not None else None
            ),
        }
