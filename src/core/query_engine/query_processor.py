...
from src.core.types import ProcessedQuery, QueryPlan
...
class QueryProcessor:
    ...
    def process(
        ...
        Args:
            ...
            query_plan: Optional orchestration plan containing expanded terms
                or decomposition metadata.
        ...
        return ProcessedQuery(
            ...
            keywords=keywords,
            filters=filters,
            expanded_terms=self._normalize_expanded_terms(query_plan),
            orchestration_mode=(
                query_plan.mode if query_plan is not None else "direct"
            ),
            sub_queries=self._normalize_sub_queries(query_plan),
            planner_reason=(
    ...
    def _normalize_expanded_terms(
        self,
        query_plan: Optional[QueryPlan],
    ) -> List[str]:
        """Normalize expanded terms from a query plan."""
        if query_plan is None or query_plan.mode != "synonym":
            return []

        seen: Set[str] = set()
        normalized: List[str] = []
        for term in query_plan.expanded_terms:
    ...
    def _normalize_sub_queries(
        ...
        """Extract plain sub-query strings from a query plan."""
        if query_plan is None or query_plan.mode != "decomposition":
            return []
        return [item.query for item in query_plan.sub_queries if item.query.strip()]
