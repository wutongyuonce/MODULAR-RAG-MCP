...
class RetrievalSettings:
    ...
    fusion_top_k: int
    rrf_k: int
    query_orchestration: QueryOrchestrationSettings = field(
        default_factory=QueryOrchestrationSettings
    )
...
class Settings:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Settings":
        ...
        query_orchestration = retrieval.get("query_orchestration") or {}
        if not isinstance(query_orchestration, dict):
            raise SettingsError(
                "Expected mapping for field: retrieval.query_orchestration"
            )
                ...
                query_orchestration=QueryOrchestrationSettings(
                    enabled=(
                        _require_bool(
                            query_orchestration,
                            "enabled",
                            "retrieval.query_orchestration",
                        )
                        if "enabled" in query_orchestration
                        else False
                    ),
                    planner_provider=(
                        _require_str(
                        ...
                        if "planner_provider" in query_orchestration
                        else "llm"
                    ),
                    mode=(
                        _require_str(
                        ...
                        if "mode" in query_orchestration
                        else "off"
                    ),
                    max_sub_queries=(
                        _require_int(
                        ...
                        if "max_sub_queries" in query_orchestration
                        else 4
                    ),
                    parallel_sub_queries=(
                        ...
                        if "parallel_sub_queries" in query_orchestration
                        else True
                    ),
                    merge_fusion=(
                        _require_str(
                        ...
                        if "merge_fusion" in query_orchestration
                        else "rrf"
                    ),
                    fallback_to_direct=(
                        _require_bool(
                        ...
                        if "fallback_to_direct" in query_orchestration
                        else True
                    ),
                    planner_timeout_seconds=(
                        ...
                        if "planner_timeout_seconds" in query_orchestration
                        else 8.0
                    ),
                ),
            ),
...
def validate_settings(settings: Settings) -> None:
    ...
    if orchestration.mode not in {"off", "auto", "synonym", "decomposition"}:
        raise SettingsError(
            "retrieval.query_orchestration.mode must be one of: "
            "off, auto, synonym, decomposition"
        )
    if orchestration.planner_provider not in {"llm", "rules"}:
        raise SettingsError(
            "retrieval.query_orchestration.planner_provider must be one of: "
            "llm, rules"
        )
    if orchestration.merge_fusion not in {"rrf", "weighted_rrf"}:
        raise SettingsError(
            "retrieval.query_orchestration.merge_fusion must be one of: "
            "rrf, weighted_rrf"
        )
    if orchestration.max_sub_queries <= 0:
        raise SettingsError(
            "retrieval.query_orchestration.max_sub_queries must be > 0"
        )
    if orchestration.planner_timeout_seconds <= 0:
        raise SettingsError(
            "retrieval.query_orchestration.planner_timeout_seconds must be > 0"
        )
