...
@dataclass
class SubQueryPlan:
    """Represents a single orchestrated sub-query generated by a planner.

    Attributes:
        query: The actual sub-query text to execute.
        intent: Short description of why this sub-query exists.
        order: Stable order for display and optional sequential execution.
    """
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "SubQueryPlan":
        """Create SubQueryPlan from dictionary."""
        return cls(**data)


@dataclass
class QueryPlan:
    """Represents a structured planning result for query orchestration.

    Attributes:
        mode: Selected orchestration mode (direct/synonym/decomposition).
        reason: Human-readable explanation for the chosen mode.
        expanded_terms: Synonym or alias terms selected by the planner.
        sub_queries: Structured sub-query plans for decomposition mode.
        confidence: Optional planner confidence score in [0, 1].
        used_fallback: Whether the planner result was produced via fallback.
    """
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "QueryPlan":
        ...
        sub_queries = [
            item if isinstance(item, SubQueryPlan) else SubQueryPlan.from_dict(item)
            for item in raw_sub_queries
        ]
        return cls(
            mode=data.get("mode", "direct"),
            reason=data.get("reason", ""),
            expanded_terms=list(data.get("expanded_terms", [])),
...
class ProcessedQuery:
    ...
    Attributes:
        ...
        orchestration_mode: Selected orchestration mode for this query
        sub_queries: Optional sub-query strings for decomposition mode
        planner_reason: Optional explanation returned by the planner
        planner_confidence: Optional planner confidence score
