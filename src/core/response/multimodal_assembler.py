...
class MultimodalAssembler:
    ...
    def __init__(
        ...
        allowed_images_root: Optional[Union[str, Path]] = None,
    ) -> None:
        ...
        self._allowed_images_root = (
            Path(allowed_images_root).resolve()
            if allowed_images_root is not None
    ...
    def resolve_image_path(
        ...
        if ref.file_path:
            try:
                path = resolve_path_under_base(ref.file_path, self._allowed_images_root)
            except ValueError:
                logger.warning("Rejected image path outside allowed root: %s", ref.file_path)
            else:
                if path.exists():
                    return str(path)
        ...
        if self._image_storage is not None:
            try:
                ...
                if path:
                    resolved = resolve_path_under_base(path, self._allowed_images_root)
                    if resolved.exists():
                        return str(resolved)
        ...
        if collection:
            try:
                ...
                logger.warning("Rejected invalid collection name during image path resolution: %s", collection)
                return None
            for ext in [".png", ".jpg", ".jpeg", ".webp"]:
                candidate = resolve_path(f"data/images/{safe_collection}/{ref.image_id}{ext}")
                if candidate.exists():
    ...
    def load_image(
        ...
        """
        try:
            path = resolve_path_under_base(file_path, self._allowed_images_root)
            if not path.exists():
