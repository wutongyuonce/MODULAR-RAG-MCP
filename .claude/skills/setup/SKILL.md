...
### Batch 4: Query Orchestration (Optional Enhancement)

Query Orchestration is an advanced feature that enables intelligent query planning — the server can automatically expand query terms (synonym expansion) or decompose complex questions into multiple sub-queries for parallel retrieval and merged results.

**Default: Disabled** (zero impact on existing query pipeline).

Ask:

1. **Enable Query Orchestration?**
   - Options: `No (disable, default)`, `Yes (enable)`
   - Recommended: `No` — enable only when you need advanced query rewriting/decomposition and have a reliable LLM endpoint

2. **If enabled — Planner Provider:**
   - Options: `LLM-based (auto-detect strategy)`, `Rules-based (simple keyword expansion)`
   - Recommended: `LLM-based` — uses the main LLM to decide whether to do direct search, synonym expansion, or logical decomposition
   - Rules-based is lightweight but only supports synonym expansion

3. **Default mode:**
   - Options: `auto (LLM decides)`, `synonym (word-level expansion)`, `decomposition (split into sub-queries)`
   - Recommended: `auto`

4. **Max sub-queries** (if decomposition mode): default `4`
5. **Parallel sub-queries**: default `true`
...
retrieval:
  query_orchestration:
    ...
    parallel_sub_queries: true   # run sub-queries in parallel
    merge_fusion: rrf            # rrf | weighted_rrf
    fallback_to_direct: true     # fallback to direct search on planner error
...
- If Query Orchestration disabled: set `retrieval.query_orchestration.enabled: false` and `mode: "off"` (default, zero intrusion)
- If Query Orchestration enabled: set `retrieval.query_orchestration.enabled: true` with the chosen `planner_provider`, `mode`, and other parameters
...
If Query Orchestration is enabled, add:
```
📋 Query Orchestration is ENABLED. The server will automatically:
   - Decide whether to expand or decompose your query
   - Merge results from multiple sub-queries (when decomposition is used)
   - Fall back to direct search on planner errors
   Check "Overview" → "Query Orchestration" card and "Query Traces" for details.
```
